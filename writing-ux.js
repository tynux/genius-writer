...
class WritingUX {
    ...
    async loadChapters() {
        ...
        // 优先从后端加载
        if (this.novelData && this.novelData.id) {
            try {
                const response = await fetch(`/api/novels/${this.novelData.id}/chapters?limit=1000`);
                const result = await response.json();
                if (result.success && result.chapters && result.chapters.length > 0) {
                    console.log('🌐 从后端加载了章节数据');
                    this.chapters = result.chapters.map(c => ({
                        id: c.id,
                ...
                console.warn('⚠️ 从后端加载章节失败:', error);
            }
        }
    ...
    updateChapterInfo(chapter) {
        ...
        // 更新状态显示
        const statusEl = document.getElementById('chapter-status-display');
        if (statusEl) {
            const statusText = {
                'planned': '待创作',
                'writing': '创作中',
    ...
    updateProgressStats() {
        ...
        if (progressLabelEl) {
            progressLabelEl.textContent = `${completedChapters}/${totalChapters} 章完成`;
        }
        
        const statChaptersEl = document.getElementById('stat-chapters');
        if (statChaptersEl) {
            statChaptersEl.textContent = completedChapters;
        }
        
        const statWordsEl = document.getElementById('stat-words');
        if (statWordsEl) {
            const totalWords = this.chapters.reduce((sum, c) => sum + (c.wordCount || 0), 0);
            statWordsEl.textContent = totalWords;
        }
    ...
    updateProgressBar(percent) {
        const progressBarEl = document.querySelector('.ring-progress');
        if (progressBarEl) {
    ...
    initEventListeners() {
        ...
        // 创作按钮
        const writeBtn = document.getElementById('write-chapter');
        if (writeBtn) {
        ...
        // 快速操作按钮
        const quickOutlineBtn = document.getElementById('quick-outline');
        if (quickOutlineBtn) {
            quickOutlineBtn.addEventListener('click', () => {
        ...
        if (quickFeedbackBtn) {
            // 修改按钮文本为“版本历史”
            const label = quickFeedbackBtn.querySelector('.action-label');
            if (label) label.textContent = '版本历史';
            const icon = quickFeedbackBtn.querySelector('.action-icon');
            if (icon) {
                icon.className = 'fas fa-history action-icon';
            }
        ...
        try {
            ...
            if (result.success) {
                ...
                if (revisions.length === 0) {
                    this.showSuccess('暂无历史版本');
                    return;
                }

                let revisionList = revisions.map(rev => 
                    `版本 ${rev.revision_number} (${new Date(rev.created_at).toLocaleString()}) - ${rev.changes_summary || '无说明'}`
                ).join('\n');

                const choice = prompt(`历史版本列表：\n${revisionList}\n\n请输入要恢复的版本号（如 1）：`);
                if (choice) {
                    const revToRestore = revisions.find(r => r.revision_number == choice);
                    if (revToRestore) {
                        const confirmRestore = confirm(`确定要恢复到版本 ${choice} 吗？当前内容将被备份。`);
                        if (confirmRestore) {
                            const restoreResp = await fetch(`/api/revisions/${revToRestore.id}/restore`, { method: 'POST' });
                            const restoreResult = await restoreResp.json();
                            if (restoreResult.success) {
                                this.showSuccess(restoreResult.message);
            ...
            console.error('❌ 获取修订版本失败:', error);
            this.showError('获取版本历史失败');
        }
    }
    ...
    exportCurrentChapter() {
        ...
        a.href = url;
        a.download = `第${chapter.number}章_${chapter.title}.md`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        
        this.showSuccess('章节已导出为 Markdown 格式');
    }
    ...
    countWords(text) {
        ...
        // 英文单词统计
        const englishWords = text.replace(/[\u4e00-\u9fff\u3000-\u303f\uff00-\uffef]/g, ' ').split(/\s+/).filter(w => w.length > 0);
    ...
    async saveCurrentChapter() {
        ...
        try {
            ...
            // 同步到后端
            if (this.novelData && this.novelData.id) {
                const response = await fetch(`/api/novels/${this.novelData.id}/chapters/${this.currentChapter}`, {
                    method: 'PUT',
