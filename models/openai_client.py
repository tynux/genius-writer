...
class OpenAIClient:
    ...
    def __init__(self, api_key: str = None, base_url: str = None):
        ...
        if not self.api_key:
            logger.warning("OpenAI API密钥未配置，将使用模拟模式")
            self.simulated = True
        else:
    ...
    def generate(self, 
        ...
        try:
            ...
            logger.info(f"OpenAI生成成功，使用 {result['usage']['total_tokens']} tokens")
            return result
...
在真实环境中，这将是由{model}模型生成的高质量内容。"""
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.generate(test_prompt, model="gpt-3.5-turbo", max_tokens=50)
...
# 工厂函数
def create_openai_client(api_key: str = None, base_url: str = None) -> OpenAIClient:
    """创建OpenAI客户端实例"""
